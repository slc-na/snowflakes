...
| `./build.ps1` | Production build with updater signing keys set as env vars, then `npm run tauri build` |

When in doubt: `npm run check` (no separate `tsc` or `svelte-check` direct call). There is no test runner or test suite in this repo.
